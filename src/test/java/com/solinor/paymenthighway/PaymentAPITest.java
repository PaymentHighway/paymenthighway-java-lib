...
public class PaymentAPITest {
	...
	public void testInitTransaction() {
        ...
        PaymentAPI paymentService = new PaymentAPI(serviceUrl, signatureKeyId, signatureSecret);
		...
		nameValueHeaders.add(new BasicNameValuePair("sph-merchant", "test_merchantId"));
        nameValueHeaders.add(new BasicNameValuePair("sph-timestamp", PaymentHighwayUtility.getUtcTimestamp()));
        nameValueHeaders.add(new BasicNameValuePair("sph-request-id", PaymentHighwayUtility.createRequestId()));
	...
	public void testDebitTransaction() {
        ...
        try {
			transactionResponse = paymentService.debitTransaction(nameValueHeaders, transactionId, transaction);
		} catch (IOException e) {
			e.printStackTrace();	
		}
	...
	public void testRevertTransaction() {
		...
		revertTransaction.setAmount("9999"); // this is optional, will default to full amount
		revertTransaction.setBlocking(true); // this is optional, will default to true
		
		TransactionResponse revertResponse = null;
		try {
			revertResponse = paymentService.revertTransaction(nameValueHeaders, transactionId, revertTransaction);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void testTransactionStatus() {
        ...
        // revert transaction 
        RevertTransactionRequest revertTransaction = new RevertTransactionRequest();
		revertTransaction.setAmount("9950"); // this is optional, will default to full amount
		revertTransaction.setBlocking(true); // this is optional, will default to true
        ...
        // status response test
     	...
     	try {
     		statusResponse = paymentService.transactionStatus(nameValueHeaders, transactionId);
     	} catch (IOException e) {
     		e.printStackTrace();
     	}
